...
public class Transitions implements RemoteCallable<Transitions> {
    ...
    public static ShellTransitions createEmptyForTesting() {
        return new ShellTransitions() {
            /* class com.android.wm.shell.transition.Transitions.AnonymousClass1 */
        };
    }
    ...
    public void onTransitionReady(IBinder iBinder, TransitionInfo transitionInfo, SurfaceControl.Transaction transaction, SurfaceControl.Transaction transaction2) {
        if (ShellProtoLogCache.WM_SHELL_TRANSITIONS_enabled) {
            ...
            ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TRANSITIONS, 1070270131, 0, "onTransitionReady %s: %s", valueOf, valueOf2);
        }
        ...
        if (findActiveTransition < 0) {
            ...
            if (ShellProtoLogCache.WM_SHELL_TRANSITIONS_enabled) {
                ...
                ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TRANSITIONS, 410592459, 0, "Invalid root leash (%s): %s", valueOf3, valueOf4);
            }
            transaction.apply();
            onAbort(iBinder);
        } else {
    ...
    private void onAbort(IBinder iBinder) {
        ...
        if (findActiveTransition >= 0) {
            if (ShellProtoLogCache.WM_SHELL_TRANSITIONS_enabled) {
                String valueOf = String.valueOf(iBinder);
                ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TRANSITIONS, 977740158, 0, "Transition animation aborted due to no-op, notifying core %s", valueOf);
            }
    ...
    private void onFinish(IBinder iBinder, WindowContainerTransaction windowContainerTransaction, WindowContainerTransactionCallback windowContainerTransactionCallback) {
        ...
        if (findActiveTransition < 0) {
            ...
            if (ShellProtoLogCache.WM_SHELL_TRANSITIONS_enabled) {
                String valueOf = String.valueOf(iBinder);
                ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TRANSITIONS, 1739415176, 0, "Transition was merged: %s", valueOf);
            }
            ActiveTransition activeTransition = this.mActiveTransitions.get(findActiveTransition);
            ...
            if (ShellProtoLogCache.WM_SHELL_TRANSITIONS_enabled) {
                ...
                ShellProtoLogImpl.v(ShellProtoLogGroup.WM_SHELL_TRANSITIONS, 1917419979, 0, "Transition animation finished, notifying core %s", valueOf2);
            }
