package com.android.wm.shell.startingsurface;

public final /* synthetic */ class StartingWindowController$$ExternalSyntheticLambda0 implements Runnable {
    public final /* synthetic */ StartingWindowController f$0;
    public final /* synthetic */ int f$1;

    public /* synthetic */ StartingWindowController$$ExternalSyntheticLambda0(StartingWindowController startingWindowController, int i) {
        this.f$0 = startingWindowController;
        this.f$1 = i;
    }

    public final void run() {
        this.f$0.lambda$onAppSplashScreenViewRemoved$2(this.f$1);
    }
}
