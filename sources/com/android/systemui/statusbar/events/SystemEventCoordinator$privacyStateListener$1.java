...
public final class SystemEventCoordinator$privacyStateListener$1 implements PrivacyItemController.Callback {
    ...
    private final boolean uniqueItemsMatch(List<PrivacyItem> list, List<PrivacyItem> list2) {
        ...
        for (T t : list) {
            arrayList.add(TuplesKt.to(Integer.valueOf(t.getApplication().getUid()), t.getPrivacyType().getPermGroupName()));
        }
        Set set = CollectionsKt___CollectionsKt.toSet(arrayList);
        ArrayList arrayList2 = new ArrayList(CollectionsKt__IterablesKt.collectionSizeOrDefault(list2, 10));
        for (T t2 : list2) {
            arrayList2.add(TuplesKt.to(Integer.valueOf(t2.getApplication().getUid()), t2.getPrivacyType().getPermGroupName()));
        }
