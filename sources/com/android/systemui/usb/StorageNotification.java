...
public class StorageNotification extends SystemUI {
            ...
            for (VolumeRecord volumeRecord : this.mStorageManager.getVolumeRecords()) {
                if (volumeRecord.getType() == 1) {
                    ...
                    if ((findVolumeByUuid == null || !findVolumeByUuid.isMountedWritable()) && !volumeRecord.isSnoozed()) {
                        String string = this.mContext.getString(17040163, volumeRecord.getNickname());
                        String string2 = this.mContext.getString(17040162);
                        Notification.Builder extend = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302828).setColor(this.mContext.getColor(17170460)).setContentTitle(string).setContentText(string2).setContentIntent(buildForgetPendingIntent(volumeRecord)).setStyle(new Notification.BigTextStyle().bigText(string2)).setVisibility(1).setLocalOnly(true).setCategory("sys").setDeleteIntent(buildSnoozeIntent(fsUuid)).extend(new Notification.TvExtender());
    ...
    private void onDiskScannedInternal(DiskInfo diskInfo, int i) {
        ...
        String string = this.mContext.getString(17040193, diskInfo.getDescription());
        String string2 = this.mContext.getString(17040192, diskInfo.getDescription());
    ...
    private Notification onVolumeChecking(VolumeInfo volumeInfo) {
        ...
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040160, disk.getDescription()), this.mContext.getString(17040159, disk.getDescription())).setCategory("progress").setOngoing(true).build();
    }
    ...
    private Notification onVolumeMounted(VolumeInfo volumeInfo) {
        ...
        if (!disk.isAdoptable() || findRecordByUuid.isInited()) {
            ...
            String string = this.mContext.getString(17040174, disk.getDescription());
            PendingIntent buildBrowsePendingIntent = buildBrowsePendingIntent(volumeInfo);
            Notification.Builder category = buildNotificationBuilder(volumeInfo, description, string).addAction(new Notification.Action(17302453, this.mContext.getString(17040158), buildBrowsePendingIntent)).addAction(new Notification.Action(17302430, this.mContext.getString(17040187), buildUnmountPendingIntent(volumeInfo))).setContentIntent(buildBrowsePendingIntent).setCategory("sys");
            if (disk.isAdoptable()) {
        ...
        String string2 = this.mContext.getString(17040170, disk.getDescription());
        ...
        return buildNotificationBuilder(volumeInfo, description2, string2).addAction(new Notification.Action(17302834, this.mContext.getString(17040161), buildInitPendingIntent)).addAction(new Notification.Action(17302430, this.mContext.getString(17040187), buildUnmountPendingIntent)).setContentIntent(buildInitPendingIntent).setDeleteIntent(buildSnoozeIntent(volumeInfo.getFsUuid())).build();
    }

    private Notification onVolumeEjecting(VolumeInfo volumeInfo) {
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040191, disk.getDescription()), this.mContext.getString(17040190, disk.getDescription())).setCategory("progress").setOngoing(true).build();
    }

    private Notification onVolumeUnmountable(VolumeInfo volumeInfo) {
        PendingIntent pendingIntent;
        DiskInfo disk = volumeInfo.getDisk();
        String string = this.mContext.getString(17040189, disk.getDescription());
        String string2 = this.mContext.getString(17040188, disk.getDescription());
        if (isAutomotive()) {
    ...
    private Notification onVolumeRemoved(VolumeInfo volumeInfo) {
        ...
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040173, disk.getDescription()), this.mContext.getString(17040172, disk.getDescription())).setCategory("err").build();
    }
    ...
    private Notification onVolumeBadRemoval(VolumeInfo volumeInfo) {
        ...
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040157, disk.getDescription()), this.mContext.getString(17040156, disk.getDescription())).setCategory("err").build();
    }
    ...
    private void onMoveProgress(MoveInfo moveInfo, int i, long j) {
        ...
        if (!TextUtils.isEmpty(moveInfo.label)) {
            str = this.mContext.getString(17040166, moveInfo.label);
        } else {
            str = this.mContext.getString(17040169);
        }
        if (j < 0) {
            charSequence = null;
        } else {
        ...
        Notification.Builder ongoing = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302828).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(charSequence).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(charSequence)).setVisibility(1).setLocalOnly(true).setCategory("progress").setProgress(100, i, false).setOngoing(true);
    ...
    private void onMoveFinished(MoveInfo moveInfo, int i) {
        ...
        if (i == -100) {
            ...
            str2 = this.mContext.getString(17040167, bestVolumeDescription);
        } else {
            str = this.mContext.getString(17040165);
            str2 = this.mContext.getString(17040164);
        }
        ...
        Notification.Builder autoCancel = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302828).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(str2).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(str2)).setVisibility(1).setLocalOnly(true).setCategory("sys").setAutoCancel(true);
