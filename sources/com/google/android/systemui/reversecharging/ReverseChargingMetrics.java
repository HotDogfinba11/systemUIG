...
public class ReverseChargingMetrics {
    ...
    public static void logStartEvent(int i, int i2) {
    ...
    public static void logReceiverType(int i) {
        int i2 = 1;
        VendorAtom createVendorAtom = createVendorAtom(1);
        createVendorAtom.atomId = 100040;
        VendorAtomValue[] vendorAtomValueArr = createVendorAtom.values;
        if (!(i == 16 || i == 114)) {
            i2 = 0;
        }
        vendorAtomValueArr[0] = VendorAtomValue.intValue(i2);
        reportVendorAtom(createVendorAtom);
    }
}
